...
describe("buildSessionAttrs", () => {
  it("emits hashed ids when hash=true and raw=false", () => {
    ...
    expect(out["braintrust.metadata.openclaw.session_key_hash"]).toMatch(/^[0-9a-f]{16}$/);
    expect(out["braintrust.metadata.openclaw.session_id_hash"]).toMatch(/^[0-9a-f]{16}$/);
    expect(out["braintrust.metadata.openclaw.run_id_hash"]).toMatch(/^[0-9a-f]{16}$/);
  ...
  it("emits raw ids when raw=true", () => {
    ...
    expect(out["braintrust.metadata.openclaw.session_key_hash"]).toBeUndefined();
  });
...
describe("buildRunAttrs", () => {
  it("maps string fields with snake_case rename for sessionKind", () => {
    ...
    const eExtra: DiagnosticEvent = { ...e, agent: "jeffery", sessionKind: "direct" };
  ...
  it("omits string fields that are missing or empty", () => {
    const common = buildCommonAttrs({ type: "run.started" }, baseOpts);
    const out = buildRunAttrs({ type: "run.started", runId: "r1", provider: "" }, common);
...
describe("buildModelUsageAttrs", () => {
  ...
  it("puts last_call deltas into conditional attrs (only when present)", () => {
    ...
    const { conditional } = buildModelUsageAttrs(
      {
        type: "model.usage",
        lastCallUsage: { input: 11, output: 22, total: 33, cacheRead: 4, cacheWrite: 5 },
      },
      common,
    );
    expect(conditional["braintrust.metadata.openclaw.last_call.prompt_tokens"]).toBe(11);
    expect(conditional["braintrust.metadata.openclaw.last_call.completion_tokens"]).toBe(22);
    expect(conditional["braintrust.metadata.openclaw.last_call.tokens"]).toBe(33);
    expect(conditional["braintrust.metadata.openclaw.last_call.prompt_cached_tokens"]).toBe(4);
    expect(conditional["braintrust.metadata.openclaw.last_call.prompt_cache_creation_tokens"]).toBe(5);
  });
  ...
  it("computes context_used_ratio when limit > 0", () => {
    ...
    expect(conditional["braintrust.metrics.context_used_ratio"]).toBeCloseTo(0.25);
    expect(conditional["braintrust.metadata.openclaw.context_limit"]).toBe(200_000);
    expect(conditional["braintrust.metadata.openclaw.context_used"]).toBe(50_000);
  });
  ...
  it("omits context_used_ratio when limit is 0", () => {
    ...
    expect(conditional["braintrust.metrics.context_used_ratio"]).toBeUndefined();
  });
  ...
  it("includes durationMs, agentId, channel when present", () => {
    ...
    expect(conditional["braintrust.metadata.openclaw.agent_id"]).toBe("jeffery");
    expect(conditional["braintrust.metadata.openclaw.channel"]).toBe("telegram");
  });
});
...
describe("buildContextAssembledAttrs", () => {
  it("maps the full DiagnosticContextAssembledEvent field set", () => {
    ...
    expect(out["braintrust.metadata.openclaw.max_message_text_chars"]).toBe(8000);
    ...
    expect(out["braintrust.metadata.openclaw.context_token_budget"]).toBe(180_000);
...
describe("buildModelCallStartedAttrs", () => {
  it("maps provider/model/api/transport/budget/upstream-hash", () => {
    ...
    expect(out["braintrust.metadata.openclaw.context_token_budget"]).toBe(180_000);
    expect(out["braintrust.metadata.openclaw.upstream_request_id_hash"]).toBe("abc123");
  });
});
...
describe("buildModelCallCloseAttrs", () => {
  ...
  it("adds error_category + failure_kind on model.call.error", () => {
    ...
    expect(out["braintrust.metadata.openclaw.error_category"]).toBe("rate_limit");
...
describe("buildToolExecutionStartedAttrs", () => {
  it("sets span type=tool and tool_name", () => {
    const common = buildCommonAttrs({ type: "tool.execution.started" }, baseOpts);
...
describe("buildToolExecutionCloseAttrs", () => {
  ...
  it("adds error_category + error_code on tool.execution.error", () => {
    ...
    expect(out["braintrust.metadata.openclaw.error_category"]).toBe("exec_failed");
  ...
  it("falls back to deniedReason when reason is absent", () => {
    ...
    expect(out["braintrust.metadata.openclaw.blocked_reason"]).toBe("rate-limited");
  });
